...
const fetchEvent = useCallback(async () => {
  ...
  const FilterEvents = ()=> {
    ...
    setFilter({type: selectValue.map((s) =>  s.value), searchTerm: searchValue });
  ...
  return (
    <div>
      ...
      />
        ...
        />{" "}
        <button className="btn btn-outline-success ml-3 w-15 h-5" onClick={FilterEvents}>
          Поиск
        </button>
      </div>
        ...
        <table className="table table-hover m-3">
          ...
          <tbody>
            {event.map((x) => (
              ...
              <Link href={`/event/view/${x.id}`}><td>{x.name}</td></Link>
        ...
        <a href="unloadingEvent" className="btn btn-outline-success " type="submit">
            Выгрузка
            </a>
