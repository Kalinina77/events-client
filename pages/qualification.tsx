...
import { deleteQualification, getQualifications, IQualification } from "../api/qualifications";
...
const Qualification = () => {
    ...
    const ApplyFilter = ()=> {
      ...
      setFilter({SpecialityIds: selectValue.map((s) =>  s.value), searchTerm: searchValue });
          ...
          <tbody>
            {qualification.map((x) => (
              <tr key={x.id}>
                ...
                <td>
                <Link href={`/qualification/edit/${x.id}`}>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 16 16"
