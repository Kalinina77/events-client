...
const Speciality = () => {
  ...
  const fetchSpeciality = useCallback(async () => {
    try {
      const res = await getSpecialitys(filter);
      setSpeciality(res);
    } catch {}
  }, [filter]);
  useEffect(() => {
  ...
  const deleteRequest = async () => {
    try {
      const res = await deleteSpeciality(selectedIds);
      fetchSpeciality();
    } catch {}
  };
  ...
  const ApplyFilter = ()=> {
    ...
    setFilter({ searchTerm: searchValue });
  ...
  return (
    <div>
      <div className="">
        <h1 className="d-flex justify-content-center">Специальность</h1>
